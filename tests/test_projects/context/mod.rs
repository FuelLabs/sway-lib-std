...
async fn can_get_this_balance() {
    ...
    let compiled = Contract::load_sway_contract("test_projects/context/out/debug/context-abi.json", salt).unwrap();
    ...
    let compiled_2 = Contract::load_sway_contract("test_artifacts/context_caller_contract/out/debug/context_caller_contract-abi.json", salt).unwrap();
...
async fn can_get_balance_of_contract() {
    ...
    let target = testcontextcontract_mod::ContractId { value: context_id.into() };
    let asset_id = testcontextcontract_mod::ContractId { value: caller_contract_id.into() };

    let result = context_instance
        .get_balance_of_contract(target, asset_id)
        .call()
        .await
        .unwrap();
...
async fn can_get_msg_value() {
    let salt = Salt::from([0u8; 32]);
    let compiled = Contract::load_sway_contract("test_artifacts/context_caller_contract/out/debug/context_caller_contract-abi.json", salt).unwrap();
    let client = Provider::launch(Config::local_node()).await.unwrap();
...
async fn can_get_msg_id() {
    ...
    let result = instance.call_get_asset_id_with_coins(send_amount, contract_id.clone()).call().await.unwrap();
