...
use fuels_signers::util::test_helpers::{setup_address_and_coins, setup_test_provider, setup_test_provider_and_wallet};
...
async fn get_context_instance() -> (TestContextContract, ContractId, testcontextcontract_mod::ContractId) {
    ...
    let instance = TestContextContract::new(contract_id.to_string(), provider.clone(), wallet.clone());
    let sway_id = testcontextcontract_mod::ContractId { value: contract_id.into() };

    (instance, contract_id, sway_id,)
}

async fn get_caller_instance() -> (TestContextCallerContract, ContractId, testcontextcallercontract_mod::ContractId) {
    let salt = Salt::from([0u8; 32]);
    let compiled = Contract::load_sway_contract("test_projects/context/out/debug/context.bin", salt).unwrap();
    let (pk, coins) = setup_address_and_coins(1, DEFAULT_COIN_AMOUNT);
    let client = Provider::launch(Config::local_node()).await.unwrap();
    ...
    let sway_id = testcontextcallercontract_mod::ContractId { value: contract_id.into() };
...
async fn can_get_this_balance() {
    ...
    let send_amount = 42;
    caller_instance.call_get_this_balance_with_coins(send_amount, caller_sway_id).set_contracts(&[context_id]).call().await.unwrap();
...
async fn can_get_msg_value() {
    ...
    let result = caller_instance.call_get_amount_with_coins(send_amount, caller_sway_id).set_contracts(&[context_id]).call().await.unwrap();
    assert_eq!(result.value, send_amount);
}
...
async fn can_get_msg_id() {
    ...
    let result = caller_instance.call_get_asset_id_with_coins(send_amount, caller_sway_id).set_contracts(&[context_id]).call().await.unwrap();
